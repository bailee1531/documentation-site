...
function generateIndex (playbook, contentCatalog, config = {}) {
  ...
  const languageCodesOnly = languages.map((x) => (x[Object.keys(x)] ? Object.keys(x)[0] : x))

  if (languageCodesOnly.length > 1 || !languageCodesOnly.includes('en')) {
    if (languageCodesOnly.length > 1 && !lunr.multiLanguage) {
      // required, otherwise lunr.multiLanguage will be undefined
      require('lunr-languages/lunr.multi')(lunr)
    }
    // required, to load additional languages
    require('lunr-languages/lunr.stemmer.support')(lunr)
    languageCodesOnly.forEach((languageCodesOnly) => {
      if (languageCodesOnly === 'ja' && !lunr.TinySegmenter) {
        require('lunr-languages/tinyseg')(lunr) // needed for Japanese Support
      }
      if (languageCodesOnly === 'th' && !lunr.wordcut) {
        lunr.wordcut = require('lunr-languages/wordcut') // needed for Thai support
      }
      if (languageCodesOnly !== 'en' && !lunr[languageCodesOnly]) {
        require(`lunr-languages/lunr.${languageCodesOnly}`)(lunr)
      }
    })
  }
  ...
  const languageStopWordsFunctions = languages
    .filter((x) => x[Object.keys(x)[0]].extraStopWords != null)
    .map((x) => lunr.generateStopWordFilter(x[Object.keys(x)[0]].extraStopWords))

  // Registers each of the language specific stop words functions as a pipeline function
  languageStopWordsFunctions.forEach((x, i) =>
    lunr.Pipeline.registerFunction(x, 'extraStopWords-' + languageCodesOnly[i])
  )

  // Construct the Lunr index from the extracted content
  const index = lunr(function () {
    if (languageCodesOnly.length > 1) {
      this.use(lunr.multiLanguage(...languageCodesOnly))
    } else if (!languageCodesOnly.includes('en')) {
      this.use(lunr[languageCodesOnly[0]])
    }
    ...
    const stopWordFilter = languageCodesOnly.includes('en')
      ? lunr.stopWordFilter
      : lunr[languageCodesOnly[0]].stopWordFilter
    ...
    languageStopWordsFunctions.forEach((x) => this.pipeline.after(lunr.stopWordFilter, x))
